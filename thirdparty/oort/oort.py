...
class _training_selector(object):
    ...
    def __init__(self, args): #, sample_seed=233):
        ...
        self.rng.seed(233 + self.args.sample_seed)  # use values of 0,1,2, ... instead
        np2.random.seed(233 + self.args.sample_seed)
    ...
    def select_participant(self, num_of_clients, feasible_clients=None):
        ...
        '''
        # #Ahmed - handle the case when the number of feasible is less than the list length
        # if len(feasible_clients) <= num_of_clients:
    ...
    def getTopK(self, numOfSamples, training_round, feasible_clients):
        ...
        #TODO: Ahmed - temporarliy disable blacklisting clients
        #self.blacklist = []
        ...
        if len(tempPickedClients) > 0:
            ...
            # probs = [scores[key]/totalSc for key in tempPickedClients]
            #Ahmed - handle NaN in probabilities
            vals = np.array([scores[key] for key in tempPickedClients], dtype=np.float64)
            totalSc = vals.sum()
            probs = vals / totalSc

            if len(probs) > 0:
                self.exploitClients = list(np2.random.choice(tempPickedClients, exploitLen, p=probs, replace=False))
            else:
                self.exploitClients = []
        ...
        if len(_unexplored) > 0:
            ...
            #Ahmed - handle NaN in probabilities or not summing to one
            ...
            vals = np.array([init_reward[key] for key in pickedUnexploredClients], dtype=np.float64)
            unexploredSc = vals.sum()
            probs = vals / unexploredSc
            if len(probs) > 0:
                pickedUnexplored = list(np2.random.choice(pickedUnexploredClients, exploreLen, p=probs, replace=False))
